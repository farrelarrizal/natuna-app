"""
Python model 'modeltest_mostlikelyscenario_132 (8).py'
...
def global_defense_partnership():
    """
    IF THEN ELSE(Time>61, RANDOM UNIFORM(1, 2 , 1 ) , 0 )
    """
    return if_then_else(time() > 61, lambda: 0, lambda: 0)
...
def modernization_and_acquisition_of_warships():
    """
    IF THEN ELSE(Time>60, 15 , 0 )
    """
    return if_then_else(time() > 60, lambda: 0, lambda: 0)
...
def economic_approach_with_china():
    ...
    """
    return np.random.uniform(0, 0, size=())
